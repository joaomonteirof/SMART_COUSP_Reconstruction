...
class model_gen(nn.Module):
	def __init__(self, n_frames, cuda_mode):
		...
		self.features = nn.Sequential(
			...
			nn.BatchNorm2d(256),
			nn.ReLU(),
			nn.Conv2d(256, 256, kernel_size=(5,5), padding=(2,1), stride=(1,1), bias=False),
			nn.BatchNorm2d(256),
			nn.ReLU(),
		...
		self.lstm = nn.LSTM(176, 256, 1, bidirectional=True, batch_first=False)
	...
	def forward(self, x):
		...
		h0 = torch.zeros(2*1, batch_size, 256)
		c0 = torch.zeros(2*1, batch_size, 256)
